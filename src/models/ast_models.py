...
def forward_att(self, x):
    '''
    function to overide the forward method of an attention block
    so that it returns the attention matrix, along with the features
    '''
    self.attn.forward_att_att = types.MethodType(forward_att_att, self.attn)
    x, attention = self.attn.forward_att_att(self.norm1(x))
    x = x + self.drop_path(x)
    x = x + self.drop_path(self.mlp(self.norm2(x)))
    return x, attention

def forward_att_att(self, x):
    '''
    function to overide the attention mechanism (in block) so that
    it returns the attention matrix
    '''
    B, N, C = x.shape
    ...
    attn_to_save = attn.softmax(dim=-1)
    attn = self.attn_drop(attn_to_save)
...
class ASTModel(nn.Module):
    ...
    def finetuningavgtok(self, x, pca_proj=False, return_attention=False):
        if return_attention:
            warnings.warn('Visualising the cls attention but the cls is not used for prediction so dodgy signal')
        B = x.shape[0]
    ...
    def finetuningcls(self, x, pca_proj=False, return_attention=False):
        '''
        return_attention: used when you want to visualise the 
        attention maps
        '''
        B = x.shape[0]
        x = self.v.patch_embed(x)
    ...
    def forward(self, 
        ...
        if task == 'ft_avgtok':
            return self.finetuningavgtok(x, pca_proj=pca_proj, return_attention=return_attention)
        # alternatively, use the [cls] token output as clip-level representation.
        elif task == 'ft_cls':
            return self.finetuningcls(x, pca_proj=pca_proj, return_attention=return_attention)
