...
def train(audio_model, train_loader, test_loader, args, test_type, val_dataset=None):
    ...
    if args.wandb == 'wandb':
        wandb.watch(audio_model, loss_fn, log='all')
    epoch += 1
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels) in enumerate(train_loader):
            ...
            if print_step and global_step != 0:
                ...
                # wandb log
                if args.wandb == 'wandb':
                    wandb.log({'step': global_step, 'loss':loss_meter.avg})
...
def validate(audio_model, val_loader, args, epoch, pca_proj=False, dataset=None, test_type=''):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_time = AverageMeter()
    if not isinstance(audio_model, nn.DataParallel):
        audio_model = nn.DataParallel(audio_model)
